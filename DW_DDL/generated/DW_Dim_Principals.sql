-- Safe regeneration template for DW.Dim_Principals (SCD Type 2)
-- Generated at 2026-03-05 15:02:49

-- Drop old backup if it exists (prevents rename conflict)
IF OBJECT_ID('DW.Dim_Principals_backup_20260305_150248', 'U') IS NOT NULL
    DROP TABLE [DW].[Dim_Principals_backup_20260305_150248];
GO
...
BEGIN
    ...
    EXEC sp_rename 'DW.Dim_Principals', 'Dim_Principals_backup_20260305_150248';
END
GO
...
BEGIN
    ...
    INSERT INTO [DW].[Dim_Principals] (
        [Principals_SK], [Row_Is_Current], [Row_Effective_Datetime], [Row_Expiry_Datetime],
        [Inserted_Datetime], [Updated_Datetime], [Is_Deleted], [Row_Change_Reason], [Principal_Code], [Principal_Name], [Principal_Trading_As_Name], [Principal_Address], [Principal_City], [Principal_Province], [Principal_Country]
    )
    SELECT 
        [Principals_SK], 1, GETDATE(), NULL,
        [Inserted_Datetime], [Updated_Datetime], [Is_Deleted], [Row_Change_Reason], [Principal_Code], [Principal_Name], [Principal_Trading_As_Name], [Principal_Address], [Principal_City], [Principal_Province], [Principal_Country]
    FROM [DW].[Dim_Principals_backup_20260305_150248];
...
CREATE UNIQUE NONCLUSTERED INDEX [UIX_NK_Dim_Principals_Active] ON [DW].[Dim_Principals] ([Principal_Code]) WHERE [Row_Is_Current] = 1 AND [Is_Deleted] = 0;
GO