-- Safe regeneration template for DW.Dim_Places (SCD Type 2)
-- Generated at 2026-03-05 15:02:49

-- Drop old backup if it exists (prevents rename conflict)
IF OBJECT_ID('DW.Dim_Places_backup_20260305_150248', 'U') IS NOT NULL
    DROP TABLE [DW].[Dim_Places_backup_20260305_150248];
GO
...
BEGIN
    ...
    EXEC sp_rename 'DW.Dim_Places', 'Dim_Places_backup_20260305_150248';
END
GO
...
BEGIN
    ...
    INSERT INTO [DW].[Dim_Places] (
        [Places_SK], [Row_Is_Current], [Row_Effective_Datetime], [Row_Expiry_Datetime],
        [Inserted_Datetime], [Updated_Datetime], [Is_Deleted], [Row_Change_Reason], [Place_Code], [Place_Name], [Place_Category], [Place_Sub_Category], [Place_Chain], [Place_Address], [Place_City], [Place_Province], [Place_Country], [Place_Latitude], [Place_Longitude], [Place_Contact_Number], [Place_Contact_Name], [Place_Contact_Title], [Place_Contact_Email], [Place_Website]
    )
    SELECT 
        [Places_SK], 1, GETDATE(), NULL,
        [Inserted_Datetime], [Updated_Datetime], [Is_Deleted], [Row_Change_Reason], [Place_Code], [Place_Name], [Place_Category], [Place_Sub_Category], [Place_Chain], [Place_Address], [Place_City], [Place_Province], [Place_Country], [Place_Latitude], [Place_Longitude], [Place_Contact_Number], [Place_Contact_Name], [Place_Contact_Title], [Place_Contact_Email], [Place_Website]
    FROM [DW].[Dim_Places_backup_20260305_150248];
...
CREATE UNIQUE NONCLUSTERED INDEX [UIX_NK_Dim_Places_Active] ON [DW].[Dim_Places] ([Place_Code]) WHERE [Row_Is_Current] = 1 AND [Is_Deleted] = 0;
GO