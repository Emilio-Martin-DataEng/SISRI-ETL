...
# Check staging tables for dimensions
tables = ['Products', 'Brands', 'Principals', 'Wholesalers', 'Places']
for table in tables:
    try:
        cursor.execute(f'SELECT COUNT(*) FROM [ODS].[{table}]')
        count = cursor.fetchone()[0]
        print(f'[ODS].[{table}] staging: {count} rows')
    except Exception as e:
        print(f'[ODS].[{table}] staging: ERROR - {e}')
